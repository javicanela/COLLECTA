# Collecta Abre-Facil Version Final

## Objetivo

Dejar Collecta operable como PWA instalable para un despacho contable: abre en navegador, puede instalarse como app, conserva shell offline, permite preparar Smart Import con archivos locales y sincroniza cartera/cobranza/reportes cuando el backend esta disponible.
...
Si Docker no esta disponible pero ya existe una DB local segura, puede omitirse la preparacion automatica:
...
Collecta incluye `manifest.webmanifest` y `service-worker.js`. El service worker conserva el shell y assets visitados para que la app no quede rota al recargar sin conexion.
...
- Esta version prioriza velocidad de entrega web-first; no incluye Tauri/Electron.
- El offline actual prepara imports y conserva shell, no implementa cartera local completa.
- Las operaciones compartidas requieren backend y DB.
- WhatsApp programatico sigue opcional; el fallback manual `wa.me` es suficiente para la version inicial final.
